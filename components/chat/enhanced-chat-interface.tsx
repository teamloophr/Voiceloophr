...
export function EnhancedChatInterface({ isDarkMode, onToggleTheme }: EnhancedChatInterfaceProps) {
  ...
  // Handle text input, run semantic search, then AI response (if API key set)
  const handleSend = async () => {
    ...
    try {
      // 1) Try semantic search (server uses env OPENAI key). This works even if user has no key.
      try {
        if (!user?.id) {
          const signInMsg: Message = {
            id: (Date.now() + 1).toString(),
            content: 'Sign in to search your saved documents and resumes.',
            role: 'assistant',
            timestamp: new Date(),
            type: 'text'
          }
        ...
        const res = await fetch('/api/search', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({ query: userMessage.content, userId: user?.id })
        })
        if (res.ok) {
          const data = await res.json()
          if (Array.isArray(data?.results) && data.results.length > 0) {
            const top = data.results.slice(0, 5)
            const lines = top.map((r: any, i: number) => `${i + 1}. ${r.title || 'Untitled'}${r.metadata?.fileType ? ` (${r.metadata.fileType})` : ''}`)
            const searchMsg: Message = {
              id: (Date.now() + 1).toString(),
              content: `I found ${data.results.length} relevant item${data.results.length === 1 ? '' : 's'}:\n${lines.join('\n')}`,
              role: "assistant",
              timestamp: new Date(),
              type: "text"
            }
            ...
            // Fetch details for the top result and include a signed link
            try {
              const topId = top[0]?.id
              if (topId) {
                const detailRes = await fetch(`/api/documents/${topId}`)
                if (detailRes.ok) {
                  const detail = await detailRes.json()
                  const detailMsg: Message = {
                    id: (Date.now() + 2).toString(),
                    content: `${detail.title}\n\n${detail.summary || ''}\n\n${detail.signedUrl ? 'View original: ' + detail.signedUrl : ''}`.trim(),
                    role: "assistant",
                    timestamp: new Date(),
                    type: "text"
                  }
            ...
            const noneMsg: Message = {
              id: (Date.now() + 1).toString(),
              content: 'I could not find a saved document matching your request. Try rephrasing, or upload the document first.',
              role: 'assistant',
              timestamp: new Date(),
              type: 'text'
            }
        ...
        console.warn('Search failed or unavailable', e)
      }

      // 2) AI response (optional; requires user API key)
      const key = getEffectiveApiKey()
      if (key) {
        const aiResponse = await AIProcessor.processHRQuery(userMessage.content, key)
        ...
        const tipMessage: Message = {
          id: (Date.now() + 2).toString(),
          content: isGuest 
            ? 'Search results shown above. To chat with the AI assistant, add your OpenAI API key in settings. As a guest, your settings are saved locally.'
            : 'Search results shown above. To chat with the AI assistant, please add your OpenAI API key in settings.',
          role: "assistant",
          timestamp: new Date(),
          type: "text"
        }
