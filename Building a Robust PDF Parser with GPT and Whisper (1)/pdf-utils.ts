...
export class EnhancedPDFProcessor {
  ...
  static async analyzePDFContent(
    ...
    try {
      ...
      if (qualityScore.isScanned && enableOCR) {
        extractionMethod = 'ocr_enhanced';
      } else if (qualityScore.hasLowTextQuality) {
   ...
   */
  private static assessContentQuality(content: string): any {
  ...
  private static async enhanceWithAI(content: string, apiKey?: string): Promise<string> {
    try {
      ...
      const prompt = `You are an expert document parser for Human Resources. Your task is to analyze the provided PDF content and extract ONLY the meaningful, human-readable text, ensuring it is clean, professional, and directly relevant to HR professionals.\n\nREQUIREMENTS:\n1.  **Remove all PDF artifacts, binary data, and technical jargon:** This includes any remnants of PDF structure (e.g., 'endstream', 'endobj', 'BT', 'ET', 'Td', 'Tj', 'TJ', 'Tm', 'Tc', 'Tw', 'Tz', 'TL', 'Ts', 'Tr', 'Ts', 'Tf'), hexadecimal codes, or any non-textual characters that would not be part of a readable document.\n2.  **Preserve the logical structure and flow:** Maintain paragraphs, headings, lists, and other formatting elements that contribute to the document's readability and comprehension. Do not flatten the content into a single block of text.\n3.  **Focus on content valuable for Human Resources professionals:** Prioritize information such as candidate details, employment history, qualifications, contract terms, policy statements, performance reviews, and any other data that an HR professional would find relevant for document management, analysis, or decision-making.\n4.  **Return clean, professional text suitable for reading aloud:** The output should be free of any extraneous characters or formatting that would hinder text-to-speech conversion or human readability. Ensure proper sentence construction and punctuation.\n5.  **Maintain document hierarchy and organization:** If the document has sections, sub-sections, or distinct logical blocks, reflect this in the extracted text to the extent possible, using appropriate line breaks or spacing.\n\nPDF Content to Process:\n${content.substring(0, 8000)}${content.length > 8000 ? '\n\n[Content truncated for processing]' : ''}\n\nPlease provide a clean, structured extraction of the document content.`
  ...
  private static async processWithOCR(content: string, apiKey?: string): Promise<string> {
    try {
      ...
      const prompt = `This content appears to be from a scanned or image-based PDF document. Your task is to perform OCR-like processing to extract and reconstruct the readable text content, focusing on information relevant to Human Resources professionals.\n\nREQUIREMENTS:\n1.  **Extract and reconstruct the readable text content:** Accurately convert image-based text into machine-readable text. Handle variations in fonts, sizes, and orientations.\n2.  **Remove all PDF artifacts, coordinates, and technical data:** Eliminate any non-textual elements, such as PDF structural markers, coordinates, or binary data that might be present due to the OCR process.\n3.  **Reconstruct sentences and paragraphs logically:** Ensure that the extracted text flows naturally, with correct sentence breaks and paragraph divisions, even if the original scan had layout challenges.\n4.  **Focus on Human Resources relevant content:** Prioritize the extraction of information pertinent to HR, such as names, addresses, contact details, employment history, educational background, skills, certifications, and any other data typically found in resumes, contracts, or HR forms.\n5.  **Return clean, professional text:** The final output should be a clean, well-formatted text document suitable for further AI analysis and human review. Avoid any extraneous characters or formatting.\n\nScanned PDF Content:\n${content.substring(0, 6000)}${content.length > 6000 ? '\n\n[Content truncated]' : ''}\n\nPlease reconstruct the readable content from this scanned document.`
